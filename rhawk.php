...
thephuckinstart:
echo "\n";
userinput("Aratmak İstediğiniz Site: ");
...
if ($ip == "help")
  {
    ...
    echo "========\n";
    echo $fgreen . "[1] help:$cln Yardım Menüsünü Görüntüleyin\n";
    echo $bold . $fgreen . "[2] fix:$cln Tüm Gerekli Modülleri Kuruyor (Aracı İlk Kez Çalıştırıyorsanız Önerilen)\n";
    echo $bold . $fgreen . "[3] URL:$cln Taramak İstediğiniz Alan Adını Girin (Format: www.sample.com / sample.com)\n";
...
elseif ($ip == "fix")
  {
    ...
    if (!extension_loaded('curl'))
      {
        echo $bold . $red . "[!] cURL Modülü yüklenme hatası ! \n";
        echo $yellow . "[*] Yükleme cURL. (Operasyon sudo izni gerektirir, bu yüzden parola istenebilir) \n" . $cln;
        system("sudo apt-get -qq --assume-yes install php-curl");
        echo $bold . $fgreen . "[i] cURL Yükleme. \n";
      }
    else
      {
        echo $bold . $fgreen . "[i] cURL zaten yüklendi, Sonrakine Atlama \n";
      }
    echo $bold . $blue . "[+] Php-XML modülü kuruluysa denetleniyor ...\n";
    if (!extension_loaded('dom'))
      {
        echo $bold . $red . "[!] php-XML Model Yükleme Hatası ! \n";
        echo $yellow . "[*] Installing php-XML. (Operasyon sudo izni gerektirir, bu yüzden parola istenebilir) \n" . $cln;
        system("sudo apt-get -qq --assume-yes install php-xml");
        echo $bold . $fgreen . "[i] DOM Yükleme. \n";
      }
    else
      {
        echo $bold . $fgreen . "[i] php-XML Zaten Yüklendi ;) \n";
      }
    echo $bold . $fgreen . "[i] İş başarıyla tamamlandı! Lütfen yeniden başla RED HAWK \n";
    exit;
  }
elseif (strpos($ip, '://') !== false)
  {
    echo $bold . $red . "\n[!] (HTTP/HTTPS) Girdide Algılandı! URL Olmadan Girt Http/Https\n" . $CURLOPT_RETURNTRANSFER;
    goto thephuckinstart;
  }
elseif (strpos($ip, '.') == false)
  {
    echo $bold . $red . "\n[!] Geçersiz URL Formatı! Geçerli Bir Girin URL\n" . $cln;
    goto thephuckinstart;
  }
elseif (strpos($ip, ' ') !== false)
  {
        ...
        elseif ($scan == "0")
          {
            ...
            echo $blue . $bold . "[i] Taranacak Site:\e[92m $ipsl" . "$ip \n";
            echo $bold . $yellow . "[S] Siteyi Tarama..." . $cln;
            echo "\n\n";
        ...
        elseif ($scan == "1")
          {
            ...
            echo "\n$cln" . $lblue . $bold . "[+] Tarama Başlıyor ... \n";
            echo $blue . $bold . "[i] Taranacak Site:\e[92m $ipsl" . "$ip \n";
            echo $bold . $yellow . "[S] Taranacak Tip : WHOIS Lookup" . $cln;
        ...
        elseif ($scan == "2")
          {
            ...
            echo "\n$cln" . $lblue . $bold . "[+] Tarama Başlıyor ... \n";
            echo $blue . $bold . "[i] Taranacak Site:\e[92m $ipsl" . "$ip \n";
            echo $bold . $yellow . "[S] Taranacak Tip : GEO-IP Lookup" . $cln;
            echo "\n\n";
        ...
        elseif ($scan == "12")
          {
            ...
            if (file_exists("crawl/admin.ini"))
              {
                echo $bold . $fgreen . "\n[^_^] Yönetici Paletli Dosya Bulundu! Yönetici Paneli İçin Tarama [-]\n" . $cln;
